...
describe('API /estoques ', () => {
  ...
  it('Obter historico de estoque por id_produto_estoque', async () => {
    const response = await request(app)
      .get(`/estoques/${1}`)
    expect(response.statusCode).toBe(200);
    expect(response.body[0].id_produto).toBe(1);
    expect(response.body[0].quantidade_estoque).toBe(0);
  });
});
