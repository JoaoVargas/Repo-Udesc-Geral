...
describe('API /estoque ', () => {
  ...
  it('Obter estoque geral', async () => {
    const response = await request(app)
      .get('/estoque')
    expect(response.statusCode).toBe(200);
    expect(response.body[0].id_produto_estoque).toBe(1);
    expect(response.body[0].estoque).toBe(0);
    expect(response.body[1].id_produto_estoque).toBe(2);
    expect(response.body[1].estoque).toBe(0);
    expect(response.body[2].id_produto_estoque).toBe(3);
    expect(response.body[2].estoque).toBe(0);
  });



  it('Obter estoque por id_produto_estoque', async () => {
    const response = await request(app)
      .get(`/estoque/${1}`)
    expect(response.statusCode).toBe(200);
    expect(response.body.id_produto_estoque).toBe(1);
    expect(response.body.estoque).toBe(0);
  });



  it('Obter estoque por id_produto_estoque, apos atuializar estoque', async () => {
    const response = await request(app)
      .get(`/estoque/${1}`)
    expect(response.statusCode).toBe(200);
    expect(response.body.id_produto_estoque).toBe(1);
    ...
    const response2 = await request(app)
      ...
      .send(updateEstoque1)
    expect(response2.statusCode).toBe(200);
    expect(response2.body.id_produto_estoque).toBe(1);
    expect(response2.body.estoque).toBe(10);


    const response3 = await request(app)
      .get(`/estoque/${1}`)
    expect(response3.statusCode).toBe(200);
    expect(response3.body.id_produto_estoque).toBe(1);
    expect(response3.body.estoque).toBe(10);
  });
});
