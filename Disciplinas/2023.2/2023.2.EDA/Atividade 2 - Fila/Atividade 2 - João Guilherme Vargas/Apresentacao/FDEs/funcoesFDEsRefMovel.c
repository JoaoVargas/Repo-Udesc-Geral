...
int insereSRM(infoNode *novo, descFDE *descritor, int (*comparaSRM)(infoNode *novo, infoNode *visitado), int *totalIteracoes) {
  ...
  comparacao = (*comparaSRM)(novo, &(descritor->cauda->dados));
...
/*************** BUSCA NA FRENTE ***************/
int buscaNaFrenteSRM(infoNode *reg, descFDE *descritor) {
...
/*************** BUSCA NA CAUDA ***************/
int buscaNaCaudaSRM(infoNode *reg, descFDE *descritor) {
  if (descritor->cauda != NULL && descritor->frente != NULL) {  // Caso exista algo na FDE
...
/*************** VAZIA? ***************/
int testaVaziaSRM(descFDE *descritor) {
  if (descritor->frente == NULL && descritor->cauda == NULL) {  // Caso exista algo na FDE
    return 1;
  }
...
/*************** TAMANHO ***************/
int tamanhoDaFilaSRM(descFDE *descritor) {
  int tam = 0;
  nodeFDE *aux = descritor->cauda;
...
/*************** PURGA ***************/
int reiniciaSRM(descFDE *descritor) {
  nodeFDE *aux = NULL;
